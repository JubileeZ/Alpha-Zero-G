---
name: orchestrator
description: End-to-end orchestrated workflow that runs a task the way Fable ran sessions: parallel evidence subagents, one committed plan, surgical execution with an intent gate, adversarial verification agents, honest outcome-first report. Use for non-trivial multi-step tasks when the user says "/orchestrator", "run the orchestrator", or "do this the way Fable would". For the rules alone without orchestration, use AGENTS.md (the Execution Protocol); for large multi-phase projects, prefer the GSD workflow and use this inside phases.
...
**Gate first.** Trivial per the Execution Protocol's triviality gate: just do it, verify with the one obvious check, report in two sentences. No stages, no subagents. Everything else runs the four stages below in order.

## Stage 1 - PLAN (the first bookend)

1. Apply core protocol Steps 0-3: classify the ask, define done with a named verification, state load-bearing assumptions.
...
2. Every edit follows core protocol Step 4: intent gate before behavior changes, recall gate before first use of anything unopened, smallest correct change, precise edits, never destroy without looking.
...
4. A surprise mid-execution re-routes per core protocol Step 2 rule 7 (backtrack edge): say it, then update the plan or go back to Stage 1. Never force the plan through a surprise.
5. Mid-item ignorance is a pause, not a guess: the moment an edit would carry a fact from memory (a signature, a key, a figure), stop that item, fan out one research subagent per the Execution Protocol's recall gate, and resume when it returns.
6. Outward-facing checklist items obey the Execution Protocol's authorization gate: no quoted user authorization, no action; the item converts to a proposed next step in the report.
...
3. A finding that survives your own check goes back to Stage 2 as new work. Hard bound per the core protocol (retry edge): 3 failed fix-verify cycles on the same issue, or any blocker outside your control, means stop and hand back with the output and your hypothesis.

## Stage 4 - AUDIT and REPORT (the second bookend)

1. Self-audit per core protocol audit mode: for each core protocol step, followed, skipped, or faked. Fix what one pass can fix (usually an unverified claim: verify it now or relabel it a caveat).
2. Deliver per core protocol Step 6: outcome in the first sentence, verification evidence shown, honest caveats, follow-ups only if they emerged from the work. No stage names or step numbers in the report; the INTENT and AUTH lines are the only method artifacts a report may contain.

## When NOT to use the orchestrator

- Trivial tasks (the gate handles them).
- Pure questions with no multi-step work: plain core protocol covers the shape.
- Inside an already-orchestrated GSD phase: GSD owns the stages there; apply core protocol rules within them instead of nesting orchestrators.
