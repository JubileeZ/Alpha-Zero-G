---
name: fable-method
description: Step-by-step problem-solving loop (classify ask, define done, gather evidence, decide, act surgically, verify by observation, report outcome-first). Use when user says "/fable-method", "use the fable method", "approach like Fable", or proactively starting multi-step task no task-specific skill covers. Subcommands: plan (stop after plan), audit (grade finished work), report (rewrite outcome-first).
trigger: /fable-method
---

# The Fable Method

Mid-tier model following this loop beats stronger free-styler: quality lives in structure, evidence, honesty, not model. Loop self-contained. Follow literally. Steps structure work, never output: do not narrate step numbers or step headers in anything user reads.
...
Deeper material on demand: `references/failure-modes.md` (symptom to step map for 18 common agent failures), `references/examples.md` (full worked examples for every ask shape), `references/domains/` (domain adapters; `domains/TEMPLATE.md` is schema; `/fable-domain` generates new), `references/flowcharts.md` (whole method as decision flowcharts; follow arrows literally when unsure how rule routes).

**Domain adapters.** Coding is default domain. If task is marketing/content, research/reporting, data analysis, business/ops, finance, legal/compliance, design/UX, or devops/infrastructure (IaC, pipelines, deploys, monitoring: script logic stays coding; live-state changes route here), read matching file in `references/domains/` before Step 2. Adapter changes only nouns, never loop: what counts as evidence, who authority is, what verification by observation means, what frauds are. **Minimum evidence set binding**: those items must actually open before acting, every time. Research never optional; adapter defines how much enough. Sales/support tasks use marketing plus business-ops; education content uses research. Medical and clinical work has no adapter on purpose: needs qualified review, not checklist; say so when asked.
...
- **Only in own inference, nothing to open or look up:** say so. Do not dress guess as rigorous process (costume, failure mode 14). Attended: ask whether to proceed with flagged low-confidence answer. Unattended: proceed but label low-confidence, never silently. No "escalate to bigger model" step; fallback everywhere is honest hand-back.
- **In specialized procedure base model lacks, recurs (or user asked reusable tooling):** build as skill via `fable-domain`.
...
- Leave behind only intended changes: delete scratch files and test artifacts created during work; note cleanup in report. Judge treats leftover debris as fraud signal; do not hand it any.
