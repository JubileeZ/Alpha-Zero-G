---
name: fable-domain
description: Discuss domain with user, research from real sources, generate trusted skill bundle: step-by-step workflow with flowchart, domain adapter, trap fixture, smoke eval. Use when user says "/fable-domain <sector>", "make a skill for <domain>", "add a domain to the fable method", "give lesser model Fable workflow for <domain>". Bundle is deliverable; workflow without flowchart, sources, trap is not done.
---

# fable-domain

fable-method ships domain adapters translating loop into sector nouns. This skill makes new one and hands user usable step-by-step **workflow with flowchart** for domain, so lesser model can approach domain way Fable would.
...
1. **Domain workflow with flowchart [v1.4].** Step-by-step approach for domain, distilled from discussion and research, plus mermaid flowchart, same shape as method's own `references/flowcharts.md`. User-facing "here are steps, in order" artifact. Lives in adapter Workflow section (see `TEMPLATE.md`).
...
**Scope stop (hard early exit, checked during discussion, before any research or generation begins).** If requested sector cannot fill template with nouns genuinely different from coding default (evidence is files and tracebacks, authority is spec, frauds are method's own failure modes), stop here and say method already covers it; no adapter generated. Debugging, refactoring, testing, general software work are default domain, not new sectors. Check lived later in generation and weak model blew straight past it, mid-build momentum winning over restraint (round 15); asked first, like red-line, costs one sentence before any work exists.
...
1. **Orient and read ALL existing adapters, not sample [observed].** Enumerate install; read every adapter in `references/domains/` plus governing docs (method SKILL.md router, fable-judge, flowcharts, README, CHANGELOG, TEMPLATE.md). Schema learned from corpus and template together.
...
5. **Wire every routing surface [observed].** Method SKILL.md adapter paragraph, flowcharts router, README adapter list and count, fable-judge sector list if enumerates sectors, CHANGELOG. Keep README and flowchart router copies byte-identical.
...
2. **Smoke eval [covenant].** Run fixture bare vs with bundle (via fable-judge suite mode, or headless harness for skill-discovery cases). One seed is smoke test, not benchmark; label it; if trap shows no difference, report bundle unproven rather than validated.
3. **Judge bundle [v1.4].** Before delivering, run fable-judge pass over bundle own claims: every named source actually fetched (spot-check at least one), trap verified in all three states (broken, wrongly fixed, correctly fixed), every routing surface actually wired, smoke eval numbers matching what runs actually showed. Bundle failing judge is not done. Exists because weak-tier makers overclaim (measured: bare Haiku called unverified bundle "production-ready", round 13); judge is backstop.
...
- User approval gates apply as in method: writing files in working copy reversible; publishing, PR-ing, or committing bundle needs user word (authorization gate).
