...
description: Discuss domain with user, research from real sources, generate trusted skill bundle: step-by-step workflow with flowchart, domain adapter, trap fixture, smoke eval. Use when user says "/adapter-builder <sector>", "make a skill for <domain>", "add a domain to the protocol", "give lesser model Fable workflow for <domain>". Bundle is deliverable; workflow without flowchart, sources, trap is not done.
trigger: /adapter-builder
---

# adapter-builder

The protocol ships domain adapters translating protocol into sector nouns. This skill makes new one and hands user usable step-by-step **workflow with flowchart** for domain, so lesser model can approach domain way Fable would.
...
1. **Domain workflow with flowchart [v1.4].** Step-by-step approach for domain, distilled from discussion and research, plus mermaid flowchart, same shape as protocol's own `references/flowcharts.md`. User-facing "here are steps, in order" artifact. Lives in adapter Workflow section (see `TEMPLATE.md`).
...
**Scope stop (hard early exit, checked during discussion, before any research or generation begins).** If requested sector cannot fill template with nouns genuinely different from coding default (evidence is files and tracebacks, authority is spec, frauds are protocol's own failure modes), stop here and say protocol already covers it; no adapter generated. Debugging, refactoring, testing, general software work are default domain, not new sectors. Check lived later in generation and weak model blew straight past it, mid-build momentum winning over restraint (round 15); asked first, like red-line, costs one sentence before any work exists.
...
flowchart TD
    ...
    SCOPE -->|no| NOAD["Stop: no adapter needed,<br/>the protocol already covers it"]
...
- User approval gates apply as in protocol: writing files in working copy reversible; publishing, PR-ing, or committing bundle needs user word (authorization gate).
