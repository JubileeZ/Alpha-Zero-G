...
**Consequences:** Corpus on disk (`evals/traps/scenarios/`, 13 Executor Traps): kept s1–s6, s9, s10, s12–s14; dropped s7, s8, s11 from the gate; added **intent-tie** and **weakened-check**. Objective scorer `score_outcome.py`. No LLM-judge override of Task Success. Vendor fable tree and `wip/fable-method` removed (git history). Historical `gate-execution-protocol-v1` incomparable.
