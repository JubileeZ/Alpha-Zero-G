...
**Status:** accepted (amended 2026-08-08 — Preview Round + Adopt Ledger @ `luna-low`); **corpus superseded by ADR 0019 (2026-08-14)** — adopt corpus = Behavior Corpus, Objective Outcome scorers, no LLM-judge pass. Runners, docker isolation, 3-arm math remain.

Treatment adopt uses the **Process Gate**: 3-arm compare (Candidate / Current / No-Harness Baseline) on the **Behavior Corpus** **and** `isolation=docker` (ADR 0013). Task Success = Observable Outcome. Report Evidence recorded separately (equivalent prose; token spelling not required). Adherence mini-campaign retired. SWE-bench Lite (ADR 0007) **superseded** — harness deleted.
...
1. **Preview Round** — full Behavior Corpus × R=1 × 3 arms → ledger `r1`. Show full detail; **always ask** before more spend.
...
**Retired:** Smoke Filter (3-id), tiered-R Adopt, `run-repeats`, tier-sweep, `classify-adopt-r`, luna-xhigh Process Gate defaults, LLM-judge as Task Success, vendored Fable S1–S14 as adopt corpus.
...
**Consequences:** Live Campaign = `evals/traps/CAMPAIGN.md`. No `evals/lite/`. Prior camps/rate research wiped for apple-to-apple. Delivery Cost never a promote input. Corpus: ADR 0019.
